...
def flush(acc: dict, writer: csv.DictWriter) -> bool:
    """Emit acc as a row if county is in scope.

    R26 (Hanwha thesis correction): keep plugged/abandoned wells. Plugged
    wells near the sale area are themselves key evidence — they show the
    drilling-era is over. plug_flag is exposed as a filterable field so
    the UI can switch between active-only and full-history views."""
