...
#
# Enforces three Readme/principles rules as executable gates so they cannot be
# skipped by a partial session-open block in WIP_OPEN.md:
#
#   - Readme §10 "branch-ahead" — fetch origin branch before creating local;
#     if remote has commits, check them out instead of reconstructing.
#   - Readme §9.7 "trust handoff recon" — detect `docs/_<slug>_handoff.md`
#     and print its contents so the executor cannot proceed without reading.
#   - principles §5 "push empty branch at session open" — if the branch is
#     new to origin, push it immediately so every subsequent commit has an
#     upstream.
#
...
# `docs/_<slug>_handoff.md` (see Readme §7.7).
#
# Exit codes:
...
# 1. Branch-ahead check (Readme §10). Fetch origin first.
...
if [ -n "$REMOTE_SHA" ]; then
  ...
  # principles §5: push empty branch immediately so origin tracks
  git push -u origin "$BRANCH"
  echo "[§5] empty branch pushed to origin with upstream tracking"
fi

# 2. Handoff-doc detection (Readme §9.7).
if [ -f "$HANDOFF" ]; then
  echo ""
  echo "=================================================================="
  echo "[§9.7] HANDOFF DOC PRESENT: $HANDOFF"
  ...
  echo ""
  echo "=================================================================="
  echo "[§9.7] END HANDOFF. Executor must apply edits per above; not re-recon."
  echo "=================================================================="
else
  echo "[§9.7] no handoff doc at $HANDOFF — fresh session"
fi
