...
**Chat 119 — DATE-RANGE FILTER for `tax_abatements.commissioned`.** Promoted from sprint queue by Chat 118: with `tax_abatements` now at 1,486 rows (up from 9), the existing text-multi-select filter over distinct ISO dates is unworkable as UX — an operator can't usefully pick from hundreds of dates. A true range slider is the natural follow-up to the LDAD ship.

### Task

1. Implement a new `date_range` filter type that renders as a min/max date slider (or paired date inputs — pick the simpler one that works on mobile per Chat 100 hit-target rules).
2. Touches three files:
   - `build.py` `compute_filter_stats`: add a branch that emits min/max ISO dates (and optionally a histogram of year buckets) when filter type is `date_range`.
   - `build_template.html` `filterFieldControlHtml`: render the new control (HTML + JS event wiring).
   - `build_template.html` matching predicate: parse the filter's two values and test `start ≤ row.commissioned ≤ end` (string comparison works for ISO dates).
3. Wire `tax_abatements.commissioned` to the new filter type in `layers.yaml`.
4. Build → deploy to prod per §8.
5. Append run log: filter-type lookup table (before/after), tax_abatements filter UX before/after.

### Acceptance

- Build clean: `built=26 missing=0 errored=0`.
- `tax_abatements` filter renders as date range, not multi-select.
- Filter actually narrows visible features when adjusted (manual smoke test on prod via curl + visual check of the rendered HTML — full JS-state verification is out of scope; if visible-features count drops correctly under different range values, accept).
- Mobile filter input still ≥40 px tall (Chat 100 sizing).
- Local↔prod md5 identical.
- Branch merged + deleted same chat per §6.12.
- No regression on the 11 other layers that currently use `text` or `numeric` filter types.

### Branch

`refinement-chat119-date-range-filter`

### Pre-flight

- Chat 118 (this chat) shipped statewide LDAD coverage: 1,486 abatement rows, 126 counties, all stamped `coords_source = ldad_county_centroid`. Build clean `built=26 missing=0 errored=0 tiles_total=12058 KB`. Local↔prod md5 identical (index `68884bbef25ad2cb97398a28041050cf`; tax_abatements tile `2aa68fd184b13981a913e2a8ce1da8c7`). Deploy `69f233aca4cf393ecf0ec587`.
- The 9 prior commissioner-court rows from `scrape_abatements.py` (Pecos/Reeves intel, Chat 83) were superseded per task acceptance ("grows from current 9 to scraped total"). LDAD typically lags commissioners-court actions by 1–2 years, so recent Pecos 2025–2026 abatement filings (CoreWeave, Poolside, Greasewood II BESS, Bighorn Ridge Solar, Silver Basin Digital, etc.) are NOT in the LDAD universe yet. If operator wants those preserved, that's a separate follow-up: re-engineer `transform_ldad.py` as overlay (drop only rows where `coords_source = ldad_county_centroid`, keep commissioner-court rows) and re-run both scrape paths. Tracked under Open backlog.
- Filter-type registry lives in `build.py compute_filter_stats` (search for the dispatch by `filter_type`). The `text` and `numeric` branches are the existing precedent; `date_range` is a third type, not a refactor of either. The matching predicate in `build_template.html` lives near the existing filter-test loop; both edits must land together.
- Mobile sizing: `.filter-text` / `.filter-range input` already at 40px (Chat 100). New control should match. Apple HIG ≥40px is acceptable; Chat 100 documented this as below the strict 44px WCAG bar but functional.
- ERCOT Stage 3 still alive in sprint queue with "resumes on CSV arrival" gate; not touching this chat.
...
**UI/UX:**
- `date_range` filter type — in flight as Chat 119 (promoted from sprint queue by Chat 118 because the existing `text` multi-select is unworkable at the new 1,486-row `tax_abatements` scale).
