...
### Caramba North OM — refresh follow-ups (2026-08-19)

OM generated by `scripts/build_caramba_om.py`. Delta vs. July v4 and rationale:
`outputs/reports/CARAMBA-OM-REBUILD-NOTES.md`. Open, highest value first:

1. **Refresh `data/datacenters/dc_anchors.json`** — 8 entries, 2 within 60 mi, compiled
   2026-04-25. Missing Chevron WTPP, TPL+Bolt JV, LandBridge Alpha Digital, Longfellow
   cluster — labelled on Exhibit 7.1 with no record behind them. Holds §7 at 9.7 GW vs ~25 GW.
2. **ERCOT queue layer is a March-2026 snapshot** — no fetch script exists (unlike EIA-860 /
   USWTDB / TCEQ / FCC). §4 static five months.
3. **RULE H boundary cases** — RRC re-stamped the two 2025 Mongoose Viper wells (6.94/6.95 mi)
   to spud 2026 / completion 2025; new-drill ≤10 mi moved 3→1. Decision: treat
   `spud_year − completion_year <= 1` as a paperwork boundary? Also feeds the map filter.
