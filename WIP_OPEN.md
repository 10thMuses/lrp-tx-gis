...
**Chat 120 — ERCOT QUEUE PRECISE GEOCODING Stage 3 (resumes on CSV arrival) OR substitution.** Stage 3 has been gated for six consecutive next-chat slots (114–119) on the operator-curated override CSV at `data/ercot_queue_overrides.csv`. The moment that CSV exists in the repo, this is unblocked. Per Chat 117 precedent, if CSV is still absent at session-open, substitute one of the unblocked items below.
...
**UI/UX:**
- `date_range` filter type — **shipped Chat 119.** Wired to `tax_abatements.commissioned` only. Other layers carrying `commissioned` data (eia860_plants, eia860_battery, wind, ercot_queue) still on `text` (auto-promoted to categorical multi-select); migrating them is Chat 120 substitution candidate (b).
- `tax_abatements` `commissioned` filter min reads `1900-01-01` — LDAD source-side placeholder for unknown approval dates. Filter still narrows correctly; cleanup is `transform_ldad.py` coercion (Chat 120 substitution candidate (c)).
