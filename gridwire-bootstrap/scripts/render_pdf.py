...
def parse_front_matter(text):
    ...
    if missing:
        sys.exit(f"render_pdf: front matter missing keys: {', '.join(missing)}")
    return meta, text[m.end():]


def preprocess(md_text):
    # ::: angle ... :::  ->  raw div, inner markdown still processed
    def angle(m):
        return (
...
def main():
    ...
    out = Path(args.out) if args.out else draft.parent / (
        f"Grid_Wire_Vol{meta['vol']}_{meta['date_compact']}_"
        f"{meta['cut_slug']}_ET.pdf"
    )
    css = CSS(filename=str(TEMPLATE_CSS), font_config=FONT_CONFIG)
