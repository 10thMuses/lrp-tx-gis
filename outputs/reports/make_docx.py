...
para(doc, [("Independently confirmed against the public hydraulic-fracturing disclosure record (FracFocus, 2011–present, cross-referenced to the wellbore record to exclude re-fracs on existing wells): ", False),
           ("no new-drill frack has ever been filed within two miles of the tract, and the most recent within five miles was 2015.", True),
...
doc.add_heading("10. The public fracking record (FracFocus) confirms: no new-drill fracking jobs within two miles of the tract, ever", level=2)
para(doc, [("The Texas ", False), ("FracFocus disclosure database", True),
           (" (fracfocus.org) is the public record of every hydraulic-fracturing job that operators have filed in Texas since 2011. Every Pecos County disclosure (949 in total) was cross-referenced against the RRC wellbore record by API number to ", False),
           ("exclude re-fracs on existing wells", True),
           (" — the figures below are confirmed new-drill fracks only (a frack performed at the original completion of a newly drilled wellbore):", False)], space_after=4)
table(doc, ["Distance band from tract", "New-drill fracks (2011 – present)", "Most recent year"], [
    ["0 – 2 mi", "0", "— none, ever"],
    ["2 – 5 mi", "9", "2015 (most recent)"],
    ["5 – 10 mi", "19", "2025"],
    ["10 – 20 mi", "450", "2026"],
], row_styles={0: {'fill': 'CFECCF', 'bold': True}, 1: {'fill': 'EAF5EA'}, 3: {'fill': 'EAF5EA'}})
para(doc, [("No new-drill hydraulic-fracturing job has ever been performed within two miles of the Caramba North tract.", True),
...
add_figure(doc, "ch_fracfocus_rings.png", width_in=6.0,
           caption="New-drill fracks (FracFocus cross-referenced against the wellbore record): 0 within 2 mi (ever), 9 within 2–5 mi (all 2012–2015), 19 within 5–10 mi, 450 within 10–20 mi. Re-fracs on existing wells are excluded.")
para(doc, [("The broader Permian new-drill fracking program does exist — 478 new-drill fracks within twenty miles since 2011, dominated by the deep-horizontal unconventional players (Diamondback 194, XTO/ExxonMobil 53, Gordy 45). But that activity is concentrated well outside the 10-mile buffer, almost entirely at unconventional depths, and annual volume has been declining (39 disclosed fracks in Pecos in 2023, 6 in 2024, 21 in 2025, 11 year-to-date 2026 vs. a 2018 – 2019 peak of 113 – 167/year).", False)])
para(doc, [("This is direct, evidence-based confirmation of what the wellbore-and-production record already implied: ", False),
           ("no active new-drill hydraulic-fracturing operation is occurring at or near the Caramba North tract, and the closest disclosed new-drill frack within five miles is over a decade old.", True)], fill='DEEAF6')
